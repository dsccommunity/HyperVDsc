...
# sample vhd partition number
$VhdPartitionNumber = 2
...
xVhdD_CopyFileOrFolder -vhdPath $vhdPath -VhdPartitionNumber $VhdPartitionNumber -itemToCopy $sampletxt -relativeDestinationPath $sampleVhdDestinationPath
Start-DscConfiguration -ComputerName localhost -Path $pwd\xVhdD_CopyFileOrFolder\ -Wait -Verbose

xVhdD_CopyFileOrFolder -vhdPath $vhdPath -VhdPartitionNumber $VhdPartitionNumber -itemToCopy $samplefolder -relativeDestinationPath $sampleVhdDestinationPath
Start-DscConfiguration -ComputerName localhost -Path $pwd\xVhdD_CopyFileOrFolder\ -Wait -Verbose
...
RemoveFileOrFolderFromVHD -vhdPath $vhdPath -VhdPartitionNumber $VhdPartitionNumber -relativeDestinationPath $sampleVhdDestinationPath
...
ChangeAttribute -vhdPath $vhdPath -VhdPartitionNumber $VhdPartitionNumber -relativeDestinationPath $sampleVhdDestinationPath -attribute 'ReadOnly'
Start-DscConfiguration -ComputerName localhost -Path $pwd\RemoveFileOrFolderFromVHD\ -Wait -Verbose
...
# Create a mof file.
Sample_EndToEndXHyperV_RunningVM -baseVhdPath "C:\test_vhds\testvhd.vhd" -VhdPartitionNumber $VhdPartitionNumber -name TestMachine -unattendedFilePathToCopy C:\temp\unattended.xml
